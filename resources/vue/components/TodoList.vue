<script lang="ts" setup>
  ...
  const toggleComplete = (todo : ITodo) => {
    Inertia.put(`/todos/${todo.uuid}/toggle-complete`, {
...
<template>
  <ul v-if="todos.length">
    <li v-for="todo in todos" :key="todo.uuid" class="flex items-start justify-between px-2 py-1 -mx-2 space-x-2 hover:bg-gray-lighter">
